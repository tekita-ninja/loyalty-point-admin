...
import { formFilterTransactionSchema, TFormFilterTransaction } from "@/schema/transaction"

export function FormFilterTransaction() {
    ...
    const form = useForm<TFormFilterTransaction>({
        resolver: zodResolver(formFilterTransactionSchema),
        defaultValues: {
    ...
    function onSubmit(values: TFormFilterTransaction) {
        const params = toQueryParams(values)
        router.push('/transaction?' + params)
        form.reset();
        setDialog(false)
    }
